...
var VFile = class {
  ...
  constructor(value) {
    ...
    this.cwd = "cwd" in options ? "" : import_node_process2.default.cwd();
    this.data = {};
    this.history = [];
    this.messages = [];
    this.value;
...
function syntaxExtension(all4, extension2) {
  ...
  for (hook in extension2) {
    ...
    if (right) {
      for (code in right) {
        if (!hasOwnProperty2.call(left, code)) left[code] = [];
...
function initializeDocument(effects) {
  ...
  function writeToChild(token, endOfFile) {
    const stream4 = self2.sliceStream(token);
    if (endOfFile) stream4.push(null);
...
function subcontent(events, eventIndex) {
  ...
  while (current) {
    ...
    if (!current._tokenizer) {
      stream4 = context.sliceStream(current);
      if (!current.next) {
        stream4.push(null);
      }
      if (previous2) {
...
// src/parser.ts
function parseMarkdown(content3, toString4) {
  ...
  for (const node2 of ast.children) {
    ...
    switch (node2.type) {
      case "heading":
        parsedText = {
          text: toString4(node2).trim(),
          type: `\u6807\u9898${node2.depth}`,
          line: node2.position?.start.line,
          priority: 10 - node2.depth
        };
        break;
      case "paragraph":
        parsedText = {
      ...
      case "list":
        for (const listItem of node2.children || []) {
          const itemText = toString4(listItem).trim();
...
function getWebviewContent(parsedTexts) {
  const textItems = parsedTexts.map(
    (item, index2) => `<div class="text-item">
            ...
            <div class="translation-area">
                ...
                <div class="buttons">
                    <button class="local-translate-btn" data-index="${index2}">\u672C\u5730\u7FFB\u8BD1</button>
                    <button class="translate-btn" data-index="${index2}">AI\u7FFB\u8BD1</button>
                    <button class="save-btn" data-index="${index2}">\u4FDD\u5B58\u7FFB\u8BD1</button>
                </div>
            </div>
        </div>`
  ).join("");
  return `<!DOCTYPE html>
    ...
    <body>
        ...
        <button id="apply-all-button">\u{1F680} \u5E94\u7528\u6240\u6709\u5DF2\u4FDD\u5B58\u7684\u7FFB\u8BD1</button>
            ...
            window.addEventListener('message', function(event) {
                ...
                switch (message.command) {
                    ...
                    case 'applyComplete': {
                        ...
                        if (applyBtn) {
                            applyBtn.textContent = '\u{1F680} \u5E94\u7528\u6240\u6709\u5DF2\u4FDD\u5B58\u7684\u7FFB\u8BD1';
  ...
  async _runTools(client, params, options) {
    ...
    for (let i = 0; i < maxChatCompletions; ++i) {
      ...
      for (const tool_call of message.tool_calls) {
        ...
        if (!fn) {
          const content4 = `Invalid tool_call: ${JSON.stringify(name)}. Available options are: ${Object.keys(functionsByName).map((name2) => JSON.stringify(name2)).join(", ")}. Please try again`;
          this._addMessage({ role, tool_call_id, content: content4 });
          continue;
        } else if (singleFunctionToCall && singleFunctionToCall !== name) {
          const content4 = `Invalid tool_call: ${JSON.stringify(name)}. ${JSON.stringify(singleFunctionToCall)} requested. Please try again`;
          this._addMessage({ role, tool_call_id, content: content4 });
          continue;
        }
        let parsed;
        try {
          ...
          const content4 = error instanceof Error ? error.message : String(error);
          this._addMessage({ role, tool_call_id, content: content4 });
          continue;
        }
        const rawContent = await fn.function(parsed, this);
        const content3 = __classPrivateFieldGet(this, _AbstractChatCompletionRunner_instances, "m", _AbstractChatCompletionRunner_stringifyFunctionCallResult).call(this, rawContent);
...
function finalizeChatCompletion(snapshot, params) {
  ...
  const completion = {
    ...
    choices: choices.map(({ message, finish_reason, index: index2, logprobs, ...choiceRest }) => {
      ...
      const { content: content3 = null, function_call, tool_calls, ...messageRest } = message;
...
${str(snapshot)}`);
      ...
      return {
        ...choiceRest,
        message: { ...messageRest, content: content3, role, refusal: message.refusal ?? null },
        finish_reason,
        index: index2,
        logprobs
      };
    ...
    case "thread.message.delta":
      ...
      if (event.data.delta.content) {
        for (const content3 of event.data.delta.content) {
          ...
          if (content3.index != __classPrivateFieldGet(this, _AssistantStream_currentContentIndex, "f")) {
            ...
            __classPrivateFieldSet(this, _AssistantStream_currentContentIndex, content3.index, "f");
          }
          __classPrivateFieldSet(this, _AssistantStream_currentContent, accumulatedMessage.content[content3.index], "f");
        }
      }
      break;
    case "thread.message.completed":
    ...
    return {
      ...
      output: response.output.map((item) => {
        ...
        if (item.type === "message") {
          return {
            ...item,
            content: item.content.map((content3) => ({
              ...content3,
              parsed: null
            }))
          };
...
function parseResponse(response, params) {
  const output = response.output.map((item) => {
    ...
    if (item.type === "message") {
      const content3 = item.content.map((content4) => {
        if (content4.type === "output_text") {
          return {
            ...content4,
            parsed: parseTextFormat(params, content4.text)
          };
        }
        return content4;
      });
  ...
  Object.defineProperty(parsed, "output_parsed", {
    ...
    get() {
      for (const output2 of parsed.output) {
        ...
        for (const content3 of output2.content) {
          if (content3.type === "output_text" && content3.parsed !== null) {
            return content3.parsed;
          }
        }
      }
      return null;
...
function parseTextFormat(params, content3) {
  if (params.text?.format?.type !== "json_schema") {
    return null;
  }
  if ("$parseRaw" in params.text?.format) {
    const text_format = params.text?.format;
    return text_format.$parseRaw(content3);
  }
  return JSON.parse(content3);
}
...
function addOutputText(rsp) {
  ...
  for (const output of rsp.output) {
    ...
    for (const content3 of output.content) {
      if (content3.type === "output_text") {
        texts.push(content3.text);
      }
    }
  }
  ...
  [(_ResponseStream_params = /* @__PURE__ */ new WeakMap(), _ResponseStream_currentResponseSnapshot = /* @__PURE__ */ new WeakMap(), _ResponseStream_finalResponse = /* @__PURE__ */ new WeakMap(), _ResponseStream_instances = /* @__PURE__ */ new WeakSet(), _ResponseStream_beginRequest = function _ResponseStream_beginRequest2() {
    ...
    switch (event.type) {
      case "response.output_text.delta": {
        ...
        if (output.type === "message") {
          const content3 = output.content[event.content_index];
          if (!content3) {
            throw new OpenAIError(`missing content at index ${event.content_index}`);
          }
          if (content3.type !== "output_text") {
            throw new OpenAIError(`expected content to be 'output_text', got ${content3.type}`);
          }
          maybeEmit("response.output_text.delta", {
            ...event,
            snapshot: content3.text
          });
        }
        break;
      }
...
var OpenAI = class {
  ...
  buildURL(path2, query, defaultBaseURL) {
    ...
    const url2 = isAbsoluteURL2(path2) ? new URL(path2) : new URL(baseURL + (baseURL.endsWith("/") && path2.startsWith("/") ? path2.slice(1) : path2));
...
// src/translate.ts
async function translateText(text3, apiKey) {
  if (!apiKey) {
...
// src/applyTranslations.ts
var vscode = __toESM(require("vscode"));
async function applyAllTranslations(translationState2, parsedTexts) {
  if (translationState2.size === 0) {
  ...
  const document4 = editor.document;
  let originalText = document4.getText();
  const sortedItems = Array.from(translationState2.entries()).sort(([idA], [idB]) => (parsedTexts[idB].line ?? 0) - (parsedTexts[idA].line ?? 0));
...
${translation}`);
  ...
  const fullRange = new vscode.Range(
    document4.positionAt(0),
    document4.positionAt(document4.getText().length)
  );
  const edit = new vscode.WorkspaceEdit();
  edit.replace(document4.uri, fullRange, originalText);
  const success = await vscode.workspace.applyEdit(edit);
  if (success) {
    vscode.window.showInformationMessage(`\u6210\u529F\u5E94\u7528\u4E86 ${translationState2.size} \u6761\u7FFB\u8BD1\uFF01`);
    translationState2.clear();
  } else {
...
// src/extension.ts
var translationState = /* @__PURE__ */ new Map();
var MODEL_SERVER_URL = "http://localhost:3000/translate";
function activate(context) {
  console.log('Congratulations, your extension "leanzh-translator" is now active!');
  const disposable = vscode2.commands.registerCommand("leanzh-translator.startTranslation", async () => {
    translationState.clear();
    console.log('\u547D\u4EE4 "leanzh-translator.startTranslation" \u5DF2\u88AB\u6210\u529F\u89E6\u53D1\uFF01');
    vscode2.window.showInformationMessage("\u7FFB\u8BD1\u547D\u4EE4\u5DF2\u542F\u52A8\uFF01");
    try {
      const editor = vscode2.window.activeTextEditor;
      if (!editor) {
        vscode2.window.showErrorMessage("\u8BF7\u5148\u6253\u5F00\u4E00\u4E2A\u6587\u6863");
        return;
      }
      const document4 = editor.document;
      if (document4.languageId !== "markdown") {
        vscode2.window.showErrorMessage("\u8BF7\u786E\u4FDD\u5F53\u524D\u6587\u6863\u662FMarkdown\u683C\u5F0F");
        return;
      }
      const content3 = document4.getText();
      const { toString: toString4 } = await Promise.resolve().then(() => (init_mdast_util_to_string(), mdast_util_to_string_exports));
      const parsedTexts = parseMarkdown(content3, toString4);
      if (parsedTexts.length === 0) {
        vscode2.window.showErrorMessage("\u672A\u627E\u5230\u9700\u8981\u7FFB\u8BD1\u7684\u6587\u672C\u5185\u5BB9");
        return;
      }
      const panel = vscode2.window.createWebviewPanel(
        "translationView",
        "\u7FFB\u8BD1\u52A9\u624B - " + document4.fileName.split("/").pop(),
      ...
      panel.webview.onDidReceiveMessage(
        async (message) => {
          switch (message.command) {
            case "translate":
              const config = vscode2.workspace.getConfiguration("leanzh-translator");
            ...
            case "localTranslate":
              ...
              try {
                const response = await axios_default.post(MODEL_SERVER_URL, {
                  text: message.text
                });
                translatedText = response.data.translation_text;
              } catch (error) {
                if (axios_default.isAxiosError(error) && error.code === "ECONNREFUSED") {
                  vscode2.window.showErrorMessage('\u65E0\u6CD5\u8FDE\u63A5\u5230\u672C\u5730\u7FFB\u8BD1\u670D\u52A1\u5668\u3002\u8BF7\u786E\u8BA4\u60A8\u5DF2\u5728\u7EC8\u7AEF\u4E2D\u8FD0\u884C "node model-server.js"\u3002');
                } else {
                  console.error("\u8C03\u7528\u672C\u5730\u7FFB\u8BD1 API \u5931\u8D25:", error);
                  vscode2.window.showErrorMessage("\u672C\u5730\u7FFB\u8BD1\u5931\u8D25\uFF0C\u8BF7\u67E5\u770B\u8C03\u8BD5\u63A7\u5236\u53F0\u83B7\u53D6\u8BE6\u60C5\u3002");
                }
              }
              panel.webview.postMessage({
            ...
            case "saveTranslation":
              ...
              if (originalText) {
                ...
                vscode2.window.showInformationMessage(`\u5DF2\u5728\u5185\u5B58\u4E2D\u4FDD\u5B58\u7B2C ${message.id + 1} \u6761\u7FFB\u8BD1`);
      ...
      vscode2.window.showErrorMessage("\u7FFB\u8BD1\u5931\u8D25\uFF0C\u8BF7\u5728\u4E3B\u7A97\u53E3\u7684\u201C\u8C03\u8BD5\u63A7\u5236\u53F0\u201D\u67E5\u770B\u9519\u8BEF\u8BE6\u60C5\u3002");
    }
  });
